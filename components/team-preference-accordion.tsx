...
export default function TeamPreferenceAccordion({ competitions, selectedCompetitionIds, selectedTeamIdsByCompetition, onToggleCompetition, onToggleTeam, onFollowAllTeams, dark = false }: Props) {
  ...
  async function ensureTeamsLoaded(competitionId: string) {
    ...
    try {
      ...
      setTeamsByCompetition((current) => ({ ...current, [competitionId]: (payload.teams ?? []).sort((a, b) => a.name.localeCompare(b.name)) }));
    } catch {
      ...
      {countries.map((country) => (
        <section key={country}>
          ...
          <div className="space-y-2">
            {competitions.filter((competition) => (competition.country ?? "International") === country).map((competition) => {
              ...
              return (
                <div key={competition.id} className={`overflow-hidden rounded-xl border transition ${followed ? dark ? "border-slate-600 bg-slate-800/70" : "border-slate-300 bg-white" : dark ? "border-slate-800 bg-slate-900/30" : "border-slate-200 bg-white"}`}>
                  ...
                  {expanded && <div className={`border-t px-4 pb-4 pt-3 ${dark ? "border-slate-700" : "border-slate-100"}`}>
                    {!followed && <div className={`mb-3 rounded-lg px-3 py-2 text-xs ${dark ? "bg-slate-800 text-slate-400" : "bg-slate-50 text-slate-500"}`}>Star this competition to follow it. You can inspect its teams before deciding.</div>}
                    ...
                    {loadingIds.includes(competition.id) ? <p className={`py-2 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Loading teams…</p> : errorIds.includes(competition.id) ? <div className="flex items-center justify-between gap-3 py-2"><p className="text-sm text-amber-500">Could not load teams.</p><button type="button" onClick={() => void ensureTeamsLoaded(competition.id)} className="text-xs font-bold underline">Retry</button></div> : <>
                      ...
                      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{filteredTeams.map((team) => { const selected = selectedSet.has(team.id); return <button key={team.id} type="button" disabled={!followed} onClick={() => onToggleTeam(competition.id, team.id)} aria-pressed={selected} className={`flex min-h-11 items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-semibold transition disabled:cursor-default disabled:opacity-50 ${selected ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-900/70 text-slate-300 hover:bg-slate-700" : "bg-slate-50 text-slate-700 hover:bg-slate-100"}`}><span className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border text-[10px] ${selected ? dark ? "border-slate-950 bg-slate-950 text-white" : "border-white bg-white text-slate-900" : dark ? "border-slate-600" : "border-slate-300"}`}>{selected ? "✓" : ""}</span><span className="min-w-0 truncate">{team.name}</span></button>; })}</div>
