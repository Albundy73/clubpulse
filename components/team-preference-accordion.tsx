...
export default function TeamPreferenceAccordion({
  ...
  const hasAnyExplicitSelection = Object.values(selectedTeamIdsByCompetition).some((ids) => ids.length > 0);
  ...
  return (
    <div className="mt-3 space-y-2">
      ...
      {competitions.map((competition) => {
        ...
        const status = selectedIds.length > 0 ? `${selectedIds.length} selected` : "All teams";
        ...
        return (
          <div key={competition.id} className={`overflow-hidden rounded-2xl border ${dark ? "border-slate-700 bg-slate-800/50" : "border-slate-200 bg-white"}`}>
            ...
            >
              ...
              <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${selectedIds.length > 0
                ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white"
            ...
            {expanded && (
              <div className={`border-t px-4 pb-4 pt-3 ${dark ? "border-slate-700" : "border-slate-100"}`}>
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <p className={`text-xs ${dark ? "text-slate-400" : "text-slate-500"}`}>Leave this competition empty to follow all of its teams.</p>
                  {selectedIds.length > 0 && (
                    <button type="button" onClick={() => onFollowAllTeams(competition.id)} className={`text-xs font-bold underline ${dark ? "text-slate-300" : "text-slate-600"}`}>
                      Follow all teams
                    </button>
                  )}
                </div>
