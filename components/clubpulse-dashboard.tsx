  ...
  function toggleTeam(teamId: string) {
    setPreferences((current) => {
      if (current.teamIds.length === 0) {
        const remainingIds = availableTeams.map((team) => team.id).filter((id) => id !== teamId);
        return { ...current, teamIds: remainingIds };
      }
      const teamIds = current.teamIds.includes(teamId)
        ? current.teamIds.filter((id) => id !== teamId)
        : [...current.teamIds, teamId];
      return { ...current, teamIds: teamIds.length === availableTeams.length ? [] : teamIds };
    });
  }
...
function PreferenceForm({ preferences, competitions, teams, catalogStatus, teamCatalogStatus, onToggleCompetition, onToggleTeam, onFollowAllTeams, dark = false, onboarding = false }: { preferences: CompetitionPreferences; competitions: CompetitionOption[]; teams: Team[]; catalogStatus: CatalogStatus; teamCatalogStatus: CatalogStatus; onToggleCompetition: (id: string) => void; onToggleTeam: (id: string) => void; onFollowAllTeams: () => void; dark?: boolean; onboarding?: boolean }) {
  ...
  return <><div><span className={`text-sm font-semibold ${labelClass}`}>{onboarding && <span className="mr-2 text-slate-500">01</span>}Competitions</span>{catalogStatus === "loading" ? <p className={`mt-3 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Loading competitions…</p> : catalogStatus === "error" ? <p className="mt-3 text-sm text-amber-500">Competition catalog is unavailable.</p> : <div className="mt-3 space-y-4">{groupedCompetitions.map((group) => <div key={group.country}><div className={`mb-2 text-xs font-bold uppercase tracking-wider ${dark ? "text-slate-500" : "text-slate-400"}`}>{group.country}</div><div className="flex flex-wrap gap-2">{group.competitions.map((competition) => { const selected = preferences.competitionIds.includes(competition.id); return <button key={competition.id} type="button" onClick={() => onToggleCompetition(competition.id)} aria-pressed={selected} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${selected ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{selected && onboarding ? "✓ " : ""}⚽ {competition.name}<span className={`ml-2 text-xs ${selected ? "opacity-60" : "text-slate-400"}`}>{competition.teamCount}</span></button>; })}</div></div>)}</div>}</div>{preferences.competitionIds.length > 0 && <div className="mt-7"><div className="flex flex-wrap items-center justify-between gap-3"><span className={`text-sm font-semibold ${labelClass}`}>{onboarding && <span className="mr-2 text-slate-500">02</span>}Teams <span className="ml-1 font-normal opacity-60">optional</span></span><button type="button" onClick={onFollowAllTeams} className={`rounded-full px-3 py-1.5 text-xs font-bold ${preferences.teamIds.length === 0 ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-800 text-slate-300" : "bg-slate-100 text-slate-600"}`}>All teams</button></div>{teamCatalogStatus === "loading" ? <p className={`mt-3 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Loading teams…</p> : teamCatalogStatus === "error" ? <p className="mt-3 text-sm text-amber-500">Team catalog is unavailable.</p> : <div className={`mt-3 max-h-64 overflow-y-auto rounded-2xl border p-2 ${dark ? "border-slate-700" : "border-slate-200"}`}><div className="flex flex-wrap gap-2">{teams.map((team) => { const selected = preferences.teamIds.length === 0 || preferences.teamIds.includes(team.id); return <button key={team.id} type="button" onClick={() => onToggleTeam(team.id)} aria-pressed={selected} className={`rounded-full px-3 py-2 text-sm font-semibold transition ${selected ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{selected ? "✓ " : ""}{team.name}</button>; })}</div>{teams.length === 0 && <p className={`p-2 text-sm ${dark ? "text-slate-500" : "text-slate-400"}`}>No teams are currently stored for these competitions.</p>}</div>}</div>}</>;
}
