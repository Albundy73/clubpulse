...
function CompetitionSummaryLogo({ src }: { src?: string }) {
...
function Onboarding({ preferences, competitions, selectedCompetitions, catalogStatus, onToggleCompetition, onToggleTeam, onFollowAllTeams, onComplete }: { preferences: CompetitionPreferences; competitions: CompetitionOption[]; selectedCompetitions: CompetitionOption[]; catalogStatus: CatalogStatus; onToggleCompetition: (id: string) => void; onToggleTeam: (competitionId: string, teamId: string) => void; onFollowAllTeams: (competitionId: string) => void; onComplete: () => void }) {
  ...
  return <section className="overflow-visible rounded-3xl bg-slate-900 text-white shadow-lg"><div className="border-b border-slate-800 px-6 py-6 sm:px-8"><h1 className="max-w-2xl text-3xl font-black tracking-tight sm:text-4xl">What do you want to follow?</h1></div><div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.4fr_0.8fr]"><div><PreferenceForm preferences={preferences} competitions={competitions} catalogStatus={catalogStatus} onToggleCompetition={onToggleCompetition} onToggleTeam={onToggleTeam} onFollowAllTeams={onFollowAllTeams} dark /></div><aside className="rounded-2xl border border-slate-700 bg-slate-800/70 p-5 lg:sticky lg:top-6 lg:self-start"><div className="flex items-center justify-between gap-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400">Your dashboard</p><button onClick={onComplete} disabled={!ready} className="rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40">Apply</button></div><div className="mt-5 space-y-5"><div><div className="text-xs text-slate-400">Favorites</div><div className="mt-2 space-y-2">{selectedCompetitions.length ? selectedCompetitions.map((competition) => <div key={competition.id} className="flex items-center gap-3 rounded-xl bg-slate-900/55 px-3 py-2"><CompetitionSummaryLogo src={competition.imageUrl} /><span className="min-w-0 truncate text-sm font-semibold text-slate-200">{competition.name}</span></div>) : <span className="text-sm text-slate-500">Star at least one competition</span>}</div></div><div><div className="text-xs text-slate-400">Teams</div><div className="mt-2"><PreferenceTeamSummary preferences={preferences} competitions={selectedCompetitions} /></div></div></div></aside></div></section>;
}
