...
const defaultPreferences: CompetitionPreferences = {
  competitionIds: [],
  teamIdsByCompetition: {},
};
...
function parseCompetitionPreferences(value: unknown): CompetitionPreferences | null {
  ...
  if (Array.isArray(candidate.teamIds)) {
    const legacyTeamIds = candidate.teamIds.filter((id): id is string => typeof id === "string");
    return {
      competitionIds: candidate.competitionIds,
      teamIdsByCompetition: Object.fromEntries(candidate.competitionIds.map((competitionId) => [competitionId, legacyTeamIds])),
    };
  }
...
export default function ClubPulseDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all(preferences.competitionIds.map(async (competitionId) => {
      ...
      return { competitionId, teams: (payload.teams ?? []).sort((a, b) => a.name.localeCompare(b.name)) };
    }))
      .then((entries) => {
        const next = Object.fromEntries(entries.map(({ competitionId, teams }) => [competitionId, teams]));
        setTeamsByCompetition(next);
        setPreferences((current) => {
          const filteredSelections: Record<string, string[]> = {};
          for (const competitionId of current.competitionIds) {
            const validIds = new Set((next[competitionId] ?? []).map((team) => team.id));
            filteredSelections[competitionId] = (current.teamIdsByCompetition[competitionId] ?? []).filter((id) => validIds.has(id));
          }
          return { ...current, teamIdsByCompetition: filteredSelections };
        });
        setTeamCatalogStatus("loaded");
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (activeCompetitionId !== "all" && !preferences.competitionIds.includes(activeCompetitionId)) {
    ...
    Promise.all(preferences.competitionIds.map(async (competitionId) => {
      ...
      .then((payloads) => {
        ...
        for (const payload of payloads) {
          for (const match of payload.matches ?? []) matchesById.set(match.id, match);
          for (const team of payload.teams ?? []) teamsById.set(team.id, team);
        }
        setLiveMatches(Array.from(matchesById.values()));
        setLiveTeams(Array.from(teamsById.values()));
  ...
  const explicitSelectionCount = preferences.competitionIds.reduce((count, competitionId) => count + (preferences.teamIdsByCompetition[competitionId]?.length ?? 0), 0);
  const allTeamsCompetitionCount = preferences.competitionIds.filter((competitionId) => (preferences.teamIdsByCompetition[competitionId]?.length ?? 0) === 0).length;
  ...
  function toggleCompetition(competitionId: string) {
    setPreferences((current) => {
      ...
      const nextSelections = { ...current.teamIdsByCompetition };
      if (selected) delete nextSelections[competitionId];
      else nextSelections[competitionId] = [];
      return { competitionIds: nextIds, teamIdsByCompetition: nextSelections };
    });
  }

  function toggleTeam(competitionId: string, teamId: string) {
    setPreferences((current) => {
      const selectedIds = current.teamIdsByCompetition[competitionId] ?? [];
      const nextIds = selectedIds.includes(teamId)
        ? selectedIds.filter((id) => id !== teamId)
        : [...selectedIds, teamId];
      return {
        ...current,
        teamIdsByCompetition: { ...current.teamIdsByCompetition, [competitionId]: nextIds },
      };
    });
  }
  ...
  function followAllTeams(competitionId: string) {
    setPreferences((current) => ({
      ...current,
      teamIdsByCompetition: { ...current.teamIdsByCompetition, [competitionId]: [] },
    }));
  }
  ...
  const emptyResultsText = liveStatus === "loading"
    ? "Loading results…"
    : liveStatus === "error"
      ? "Results are unavailable because PostgreSQL could not be reached."
      : "No recent results are stored for your selected competitions and teams.";
  const emptyUpcomingText = liveStatus === "loading"
    ? "Loading upcoming games…"
    : liveStatus === "error"
      ? "Upcoming games are unavailable because PostgreSQL could not be reached."
      : "No upcoming games are stored in the next 7 days for your selected competitions and teams.";
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-5 py-8">
        {!onboardingComplete ? <Onboarding preferences={preferences} competitions={competitions} teamsByCompetition={teamsByCompetition} selectedCompetitions={selectedCompetitions} catalogStatus={catalogStatus} teamCatalogStatus={teamCatalogStatus} onToggleCompetition={toggleCompetition} onToggleTeam={toggleTeam} onFollowAllTeams={followAllTeams} onComplete={completeOnboarding} /> : <>
          ...
          <MatchSection eyebrow={activeCompetitionId === "all" ? "Selected competitions" : selectedCompetitions.find((competition) => competition.id === activeCompetitionId)?.name ?? "Selected competition"} title="Latest results" count={results.length} groups={groupMatchesByDay(results)} sportMap={sportMap} teamMap={teamMap} teamSelectionsByCompetition={preferences.teamIdsByCompetition} result emptyText={emptyResultsText} />
          <MatchSection eyebrow="Next 7 days" title="Upcoming games" count={upcoming.length} groups={groupMatchesByDay(upcoming)} sportMap={sportMap} teamMap={teamMap} teamSelectionsByCompetition={preferences.teamIdsByCompetition} emptyText={emptyUpcomingText} />
        </>}
...
function PreferenceForm({ preferences, competitions, teamsByCompetition, catalogStatus, teamCatalogStatus, onToggleCompetition, onToggleTeam, onFollowAllTeams, dark = false, onboarding = false }: { preferences: CompetitionPreferences; competitions: CompetitionOption[]; teamsByCompetition: Record<string, Team[]>; catalogStatus: CatalogStatus; teamCatalogStatus: CatalogStatus; onToggleCompetition: (id: string) => void; onToggleTeam: (competitionId: string, teamId: string) => void; onFollowAllTeams: (competitionId: string) => void; dark?: boolean; onboarding?: boolean }) {
  ...
  const selectedCompetitions = competitions.filter((competition) => preferences.competitionIds.includes(competition.id));
  return <><div><span className={`text-sm font-semibold ${labelClass}`}>{onboarding && <span className="mr-2 text-slate-500">01</span>}Competitions</span><CatalogRefreshControl dark={dark} />{catalogStatus === "loading" ? <p className={`mt-3 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Loading competitions…</p> : catalogStatus === "error" ? <p className="mt-3 text-sm text-amber-500">Competition catalog is unavailable.</p> : <div className="mt-3 space-y-4">{groupedCompetitions.map((group) => <div key={group.country}><div className={`mb-2 text-xs font-bold uppercase tracking-wider ${dark ? "text-slate-500" : "text-slate-400"}`}>{group.country}</div><div className="flex flex-wrap gap-2">{group.competitions.map((competition) => { const selected = preferences.competitionIds.includes(competition.id); return <button key={competition.id} type="button" onClick={() => onToggleCompetition(competition.id)} aria-pressed={selected} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${selected ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{selected && onboarding ? "✓ " : ""}⚽ {competition.name}<span className={`ml-2 text-xs ${selected ? "opacity-60" : "text-slate-400"}`}>{competition.teamCount}</span></button>; })}</div></div>)}</div>}</div>{preferences.competitionIds.length > 0 && <div className="mt-7"><div className="flex flex-wrap items-center justify-between gap-3"><span className={`text-sm font-semibold ${labelClass}`}>{onboarding && <span className="mr-2 text-slate-500">02</span>}Teams <span className="ml-1 font-normal opacity-60">optional</span></span><span className={`text-xs ${dark ? "text-slate-400" : "text-slate-500"}`}>No selection inside a competition = all teams</span></div>{teamCatalogStatus === "loading" ? <p className={`mt-3 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Loading teams…</p> : teamCatalogStatus === "error" ? <p className="mt-3 text-sm text-amber-500">Team catalog is unavailable.</p> : <div className="mt-3 space-y-2">{selectedCompetitions.map((competition) => { const teams = teamsByCompetition[competition.id] ?? []; const selectedIds = preferences.teamIdsByCompetition[competition.id] ?? []; const isOpen = expanded[competition.id] ?? false; const query = queries[competition.id] ?? ""; const filteredTeams = query.trim() ? teams.filter((team) => team.name.toLowerCase().includes(query.trim().toLowerCase())) : teams; return <div key={competition.id} className={`overflow-hidden rounded-xl border ${dark ? "border-slate-700 bg-slate-800/50" : "border-slate-200 bg-white"}`}><button type="button" onClick={() => setExpanded((current) => ({ ...current, [competition.id]: !isOpen }))} aria-expanded={isOpen} className={`flex w-full items-center gap-3 px-4 py-3 text-left transition ${dark ? "hover:bg-slate-800" : "hover:bg-slate-50"}`}><span className={`text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>{isOpen ? "▾" : "▸"}</span><span className={`min-w-0 flex-1 truncate text-sm font-bold ${dark ? "text-white" : "text-slate-900"}`}>⚽ {competition.name}</span><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${selectedIds.length === 0 ? dark ? "bg-slate-700 text-slate-200" : "bg-slate-100 text-slate-600" : dark ? "bg-white text-slate-950" : "bg-slate-900 text-white"}`}>{selectedIds.length === 0 ? "All teams" : `${selectedIds.length} selected`}</span></button>{isOpen && <div className={`border-t px-4 py-4 ${dark ? "border-slate-700" : "border-slate-200"}`}><div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center">{teams.length > 10 && <input value={query} onChange={(event) => setQueries((current) => ({ ...current, [competition.id]: event.target.value }))} placeholder="Search teams…" className={`min-w-0 flex-1 rounded-lg border px-3 py-2 text-sm outline-none transition focus:ring-2 ${dark ? "border-slate-600 bg-slate-900 text-white placeholder:text-slate-500 focus:border-slate-500 focus:ring-slate-700" : "border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 focus:border-slate-300 focus:ring-slate-100"}`} />}<button type="button" onClick={() => onFollowAllTeams(competition.id)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-bold ${selectedIds.length === 0 ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-700 text-slate-200 hover:bg-slate-600" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>All teams</button></div>{teams.length === 0 ? <p className={`text-sm ${dark ? "text-slate-500" : "text-slate-400"}`}>No teams are currently stored for this competition.</p> : filteredTeams.length === 0 ? <p className={`text-sm ${dark ? "text-slate-500" : "text-slate-400"}`}>No teams match “{query}”.</p> : <div className="grid gap-2 sm:grid-cols-2">{filteredTeams.map((team) => { const selected = selectedIds.includes(team.id); return <label key={team.id} className={`flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${selected ? dark ? "bg-white text-slate-950" : "bg-slate-900 text-white" : dark ? "bg-slate-900/60 text-slate-300 hover:bg-slate-700" : "bg-slate-50 text-slate-700 hover:bg-slate-100"}`}><input type="checkbox" checked={selected} onChange={() => onToggleTeam(competition.id, team.id)} className="h-4 w-4 rounded" /><span className="min-w-0 truncate">{team.name}</span></label>; })}</div>}</div>}</div>; })}</div>}</div>}</>;
}

function MatchCard({ match, sport, teamMap, selectedTeamIds, result = false }: { match: Match; sport?: (typeof sports)[number]; teamMap: Map<string, Team>; selectedTeamIds: string[]; result?: boolean }) {
  const home = teamMap.get(match.homeTeamId);
  const away = teamMap.get(match.awayTeamId);
  const followsAll = selectedTeamIds.length === 0;
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-xs sm:px-5"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white px-2.5 py-1 font-semibold text-slate-700 shadow-sm">{sport?.icon} {sport?.name}</span><span className="font-medium text-slate-500">{match.competition}</span>{match.source.provider === "thesportsdb" && <span className="rounded-full bg-blue-100 px-2 py-1 font-bold text-blue-700">TheSportsDB</span>}</div>{!result && <span className="rounded-full bg-slate-900 px-3 py-1 font-bold text-white">{formatTime(match.date)}</span>}</div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 px-4 py-5 sm:gap-6 sm:px-5"><TeamDisplay team={home} followed={followsAll || selectedTeamIds.includes(match.homeTeamId)} align="right" /><div className="min-w-20 text-center sm:min-w-24">{result ? <div className="rounded-xl bg-slate-900 px-3 py-2 text-xl font-black tracking-tight text-white sm:text-2xl">{match.homeScore} - {match.awayScore}</div> : <div><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Kick-off</div><div className="mt-1 text-lg font-black text-slate-900">{formatTime(match.date)}</div></div>}</div><TeamDisplay team={away} followed={followsAll || selectedTeamIds.includes(match.awayTeamId)} align="left" /></div>{match.venue && <div className="border-t border-slate-100 px-4 py-3 text-xs text-slate-500 sm:px-5">📍 {match.venue}</div>}</article>;
}
